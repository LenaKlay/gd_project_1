...
K = 10**1 # 1000000000       # Carrying capacity for a spatial interval of size 1
dx = 1                      # Spatial interval 
T = 500                     # Time at which simulation stops (it can stop earlier if one of the type disappear from the environment)
m = 0.2                     # Migration probability
dt = np.round(m*dx**2/2,10) # Time interval
nb_graph = 500                # Number of graphs shown if T is reached 
...
# Where to save datas
dir_save = f"stoch_save/datas/2D_{conv_timing}_K_{int(np.log10(K))}_dx_{dx}_s_{s}_r_{r}"
...
for t in np.arange(0, T, dt): 
    ...
    if t >= graph_counter*(T/nb_graph):
        fig, ax = plt.subplots()
        nDplot = np.ma.masked_where(nD==0,nD)
        nWplot = np.ma.masked_where(nW==0,nW)
        cmap = plt.cm.Reds
        cmap.set_bad(color='white',alpha=0)
        plt.imshow(nDplot,cmap = cmap, vmin = 0, vmax = K*dx/5,alpha=1)
        cmap = plt.cm.Blues
        cmap.set_bad(color='white',alpha=0)
        plt.imshow(nWplot,cmap = plt.cm.Blues,vmin = 0, vmax = K*dx, alpha=0.5)
        plt.axis('off')
        plt.tight_layout()
        ...
        fig.savefig(f"{dir_save}/nW_t_{t}.png", format='png', bbox_inches="tight")
        plt.close()
        print(graph_counter)
        graph_counter += 1

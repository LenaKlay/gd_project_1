...
############ Same file than drive_simu.py but with a loop on K and s ################
...
for K in [10**3, 10**5, 10**7]:   # 10**3, 10**5, 10**7, 10**8
...
s_nb = 9
K_nb = 3
chasing_matrix = np.ones((K_nb, s_nb))*(-1)
for i in range(K_nb):
    K = [10**3, 10**5, 10**7][i]
    for j in range(s_nb):         
        s = np.round(np.arange(0.1,1,0.1)[j],3)
        print(s)
        chasing_matrix[i,j] = np.mean(np.loadtxt(f"chasing/chasing_K_{int(np.log10(K))}_s_{s}.txt"))

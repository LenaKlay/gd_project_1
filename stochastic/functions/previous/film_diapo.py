...
K = 10**4 # 1000000000      # Carrying capacity for a spatial interval of size 1
dx = 1                      # Spatial interval 
T = 1500                    # Time at which simulation stops (it can stop earlier if one of the type disappear from the environment)
m = 0.2                     # Migration probability
dt = np.round(m*dx**2/2,10) # Time interval
nb_graph = 30                # Number of graphs shown if T is reached 
...
# Where to save datas
dir_save = f"../../../stoch_not_save/datas/diapo_{conv_timing}_K_{int(np.log10(K))}_dx_{dx}_s_{s}_r_{r}"
...
def graph(nW, nD, semilogy, grid, line_size, dir_save, graph_counter):
       
        fig, ax = plt.subplots()
        #Plot
        ax.plot(nW, color = 'cornflowerblue', label='Wild-type', linewidth = line_size)       
        ax.plot(nD, color = 'crimson', label='Drive', linewidth = line_size)
        # Delete default ticks
        ax.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
        ax.tick_params(axis='y', which='both', bottom=False, top=False, labelbottom=False)
        # Ticks and tick labels on y axis
        ax.set_yticks([0,K*dx])       
        ax.set_yticklabels(['0',f"$10^{int(np.log10(K))}$"], fontsize=number_y_size)      
        # Axis labels and y dimension
        ...
        # Show the graph     
        num = str(graph_counter)
        if len(num)==1: num = '0'+'0'+num
        if len(num)==2: num = '0'+num
        fig.savefig(f"{dir_save}/{num}.png", format='png')
        fig.savefig(f"{dir_save}/{num}.svg", format='svg')
        plt.show()
        
        


# Set the space function of the speed
...
nD = np.zeros(nb_sites).astype(int); nD[:nb_sites//4] = K*dx
nW = np.zeros(nb_sites).astype(int); nW[nb_sites//4:] = K*dx
...
for t in np.arange(0, T, dt): 
    ...
    if t >= graph_counter*(T/nb_graph):
        ...
        graph(nW, nD, semilogy, grid, line_size, dir_save, graph_counter-1)
        print(graph_counter)
        graph_counter += 1

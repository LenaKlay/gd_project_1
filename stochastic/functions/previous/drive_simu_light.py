...
####### DRAFT to save only the datas used in the histogram #############
####### Written but not strongly tested ################################
...
if dx == 1 : nb_save = int(T/dt) 
if dx == 0.1 : nb_save = 1000              # Number of values (nD, nW) saved in between 0...T (In zoom all values are saved)
...
nb_drive = 100              # for the eradication times, when dx=0.1

v_cont = erad_s(conv_timing, r, c, h) 
nb_sites = int(((v_cont*T*2/dx)//1000)*1000+1000)

# First line : index of the spatial site, second line : time at which nb or 0 is reached
index_nb = np.ones(nb_sites//2).astype('int')*(-1)
time_nb = np.ones(nb_sites//2)*(-1)
index_0 = np.ones(nb_sites//2).astype('int')*(-1)
time_0 = np.ones(nb_sites//2)*(-1)
...
for t in np.arange(0, T, dt): 
    ...
    if dx == 0.1 and t>2/3*T:
        
        if index_nb[0] == -1:
            index_nb[0] = np.where(nW >= nb_drive)[0][0]
            time_nb[0] = t          
            
        free_nb = np.where(index_nb==(-1))[0][0]
        index_wt_under_nb = index_nb[free_nb-1]+np.where(nW[index_nb[free_nb-1]:] < nb_drive)[0]
        if len(index_wt_under_nb)!=0 :
            new_index_wt = np.arange(index_nb[free_nb-1]+1,index_wt_under_nb[-1]+1).astype('int')
            index_nb[free_nb:free_nb+len(new_index_wt)] = new_index_wt
            time_nb[free_nb:free_nb+len(new_index_wt)] = t-dt
            
        free_nb = np.where(index_nb==(-1))[0][0]
        free_0 = np.where(index_0==(-1))[0][0]
        start = min(free_0-20, 0)
            
        for i in index_nb[start:free_nb]:
            if nW[i]==0 and time_0[i-index_nb[0]]==-1: 
                time_0[i-index_nb[0]] = t
            if nW[i]!=0 and time_0[i-index_nb[0]]!=-1:
                time_0[i-index_nb[0]] = -1

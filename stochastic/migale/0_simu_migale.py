...
K = 10**(K_val) # 1000000000       # Carrying capacity for a spatial interval of size 1
...
h = 0.4                     # Dominance
s = np.round(0.1*(s_val)+0.05 ,3)# Disadvantage for drive
...
for run in range(replicats):
            ...
            for t in np.arange(0, T, dt): 
                ...
                if np.where(nD==max(nD))[0][0] > len(nD)-10 or t==T-dt : 
                    ...
                    # Save datas 
                    file = open(f"res_K_{int(np.log10(K))}_s_{s}.txt", "a") 
                    file.write(f"\n{chasing}")  
                    file.close() 

...
def graph(X,W,H,D,t,speed,graph_para,r,s,L):
        ...
        # Saving figures
        directory = f"evo_r_{r}_s_{s}_semilogy_{semilogy}"
        file = "evo"
        save_figure(t, fig, directory, file) 
        columns = [X,W,D]; np.savetxt(f"../outputs/{directory}/column_t_{t}.txt", np.column_stack(columns), fmt='%.3e', delimiter="  ") 
...
# Create a new directory {dir_title} in outputs and save figures in function of time t_{t}_{figure_title}.pdf or not {figure_title}.pdf
def save_figure(t, fig, dir_title, figure_title) : 
    
    # Create new directory
    if t == 0 or t == None : 
        actual_dir = os.getcwd()
        print ("The current working directory is %s" % actual_dir)
        new_dir = f"../outputs/{dir_title}"
        try:
            os.mkdir(new_dir)
        except OSError:
    ...
    if t != None : 
        fig.savefig(f"../outputs/{dir_title}/t_{t}_{figure_title}.pdf")  
    else : 
        fig.savefig(f"../outputs/{dir_title}/{figure_title}.pdf") 

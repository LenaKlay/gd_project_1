...
def pulled_pushed_graph(X,W,D1,D2,t,bio_para,num_para,mod):
        ...
        directory = f"pulled_pushed/{conversion_timing}_r_{np.round(r,3)}_s_{np.round(s,3)}_h_{np.round(h,2)}_c_{np.round(c,2)}"
...
def pulled_pushed_wave(bio_para, num_para, graph_para) :  
    ...
    pulled_pushed_graph(X,W,D1,D2,0,bio_para,num_para,mod)
    nb_graph = 1
    ...
    for t in np.linspace(dt,T,M) :
        ...
        # Graph
        if t>=mod*nb_graph : 
            pulled_pushed_graph(X,W,D1,D2,t,bio_para,num_para,mod)
            nb_graph += 1

...
def evolution(r,s,h,difW,difH,difD,c,T,L,M,N,theta):    
    ...
    for t in np.linspace(dt,T,M) :
        ...
        f1 = growth(W,H,D,r,a)*mating(c,W,H,D)[0] - death(W,H,D,r,a)*W
        f2 = (1-s*h)*growth(H,W,D,r,a)*mating(c,W,H,D)[1] - death(W,H,D,r,a)*H
        f3 = (1-s)*growth(D,H,W,r,a)*mating(c,W,H,D)[2] - death(W,H,D,r,a)*D
...
def heatmap(precision,cmin,cmax,rmin,rmax):

    # Range for r and s
    delta_c = (cmax-cmin)/precision    # delta_s is the size of a simulation pixel (size mesured with the s scale)  
    c_range = np.arange(cmin+delta_c/2,cmax+delta_c/2,delta_c)       # s values for simulations (NB : smin and smax are not simulated, we simulate values centered on the simulation pixels)                  
    if rlog : 
    ...
    for c_index in range(0, precision) :          
            c = c_range[c_index]
            print("\nc=", np.round(c,3))           
            heatmap_vect[c_index] = evolution(r,s,h,difW,difH,difD,c,T,L,M,N,theta)
...
heatmap_vect = heatmap(precision,cmin,cmax,rmin,rmax)

...
def heatmap(precision,hmin,hmax,rmin,rmax):

    # Range for r and h
    delta_h = (hmax-hmin)/precision    # delta_s is the size of a simulation pixel (size mesured with the s scale)  
    h_range = np.arange(hmin+delta_h/2,hmax+delta_h/2,delta_h)       # s values for simulations (NB : smin and smax are not simulated, we simulate values centered on the simulation pixels)                  
    if rlog : 
...
c = 0.85     # homing rate
s = 0.95     # and sh for heterozygous individuals
...
heatmap_vect = heatmap(precision,hmin,hmax,rmin,rmax)
